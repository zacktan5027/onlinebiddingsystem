    ...
    <div class="container">
        ...
        <div class="container-content rounded shadow">
            <header>
                ...
                <h2 class="h5 text-uppercase mb-4">Items you might like</h2>
            </header>
            <div class="row">
                <?php

                $category = "";
                $sql = "SELECT COUNT(*),categoryID,category_name FROM favourite JOIN item ON favourite.itemID=item.itemID JOIN category ON item.item_category=category.categoryID WHERE userID = " . $_SESSION["user"]["id"] . " GROUP BY category_name LIMIT 1";
                $query = mysqli_query($conn, $sql);
                $categoryName = mysqli_fetch_array($query);

                if (mysqli_num_rows($query) > 0) {
                    $category = "AND item_category=" . $categoryName["categoryID"];
                }

                $sql = $conn->query("SELECT * FROM bidding NATURAL JOIN item WHERE bidding_status='start' $category AND item_status=1 AND sellerID!=" . $_SESSION["user"]["id"] . " ORDER BY RAND() DESC LIMIT 8");
                $items = [];
                ...
                ?>
                    <div class="col-xl-3 col-lg-4 col-sm-6">
                        <div class="product text-center my-2">
